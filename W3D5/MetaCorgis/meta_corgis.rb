...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    ...
    if method_name.start_with?("get_")
      attributes_str = method_name["get_".length..-1]
      attributes_names = attributes_str.split("_")
      search_conditions = {}

      attributes_names.length.times do |i|
        search_conditions[i] = attributes_names[i]
      end

      if attributes_names.length > 2
